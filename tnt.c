...
void module_init(void **data)
{
	...
	s.active_tnt = register_blocktype(-1, "active_tnt", &convert_active_tnt, &trigger_active_tnt, NULL);
	s.explosion = register_blocktype(-1, "explosion", &convert_explosion, NULL, &physics_explosion);
	s.fuse = register_blocktype(-1, "fuse", &convert_fuse, &trigger_fuse, &physics_fuse);
}
