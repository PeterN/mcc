...
int trigger_spleef(struct level_t *level, unsigned index, const struct block_t *block)
{
...
static int trigger_spleeft(struct level_t *level, unsigned index, const struct block_t *block)
{
	int16_t x, y, z;
	level_get_xyz(level, index, &x, &y, &z);
...
void module_init(void **data)
{
	...
	s.spleef1 = register_blocktype(BLOCK_INVALID, "spleef1", RANK_BUILDER, &convert_spleef1, &trigger_spleef, NULL, &physics_spleef, false);
	s.spleef2 = register_blocktype(BLOCK_INVALID, "spleef2", RANK_BUILDER, &convert_spleef2, &trigger_spleef, NULL, &physics_spleef, false);
	s.spleeft = register_blocktype(BLOCK_INVALID, "spleeft", RANK_BUILDER, &convert_spleeft, &trigger_spleeft, NULL, NULL, false);
